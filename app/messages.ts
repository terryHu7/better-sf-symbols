...
const zh: Messages = {
  ...
  meta: {
    ...
    // 动词「预览」，外加两个只有中文侧才有的入口词——「苹果开发者」和「Vibe Coding」，
    // 后者跟着 README 的写法（大写、带空格），别在这里另起一套。
    //
    // 这一版说的是**场景**不是机制（2026-08-20 由作者定，理由是上一版中文太拧巴）。
    // 花掉的是「真图并排铺开」那半句——那正是 SF Symbols.app 结构上给不了的东西
    // （见 CLAUDE.md 「对比」），但它和「一网打尽 vs 对比」是同一个取舍：并排是用过
    // 一次之后才认得的好处，**不是把人领进门的那句话**。搜索结果里的人认得的是
    // 「vibe coding 时挑图标」这件苦差事，所以描述里站的是那一头。
    description:
      "面向苹果开发者 Vibe Coding 时挑选按钮图标的场景，输入整段 AI 回复的文本，自动提取其中的图标符号并预览出来。",
